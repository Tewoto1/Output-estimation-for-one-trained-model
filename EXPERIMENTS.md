...
| `checkpoints/affine_conditional_layer1/` | **affine-conditional layer-1 "why"** notebook (`experiments/affine_conditional_layer1/`) — ONE layer `M = W + e₁e₁ᵀ` (**no training**), `X~N(0,I)`, conditioned on the spike coord `s`: bulk-given-`s` post-ReLU mean/var/off-diag computed via EXACT rectified-Gaussian integrals (no MC, no binning). `aff_l1_*.npz` per-(width,seed) fit+Taylor rows (widths 64–4096, seeds 0–4), `mc_l1_*.npz` MC validation, `figs/` PNGs, `summary_rows.json`. Result: affine-in-`s` 1−R² ∝ 1/n with closed-form constants (mean `2φ(0)²β²τ²/σ²`; var 3.07× smaller; slope dir = Φ(0)β exactly); cond. variance tilts at `2φ(0)σ` × mean slope — affine, not constant |
