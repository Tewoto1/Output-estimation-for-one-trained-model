...
| `checkpoints/analytic_kprop/` | **analytic-kprop** notebook + `experiments/analytic_kprop/binning_scaling_experiment.py` (A100 width-scaling, ARC-infra runnable) + **`affine_r2_colab.ipynb`** (affine-hypothesis 1−R² vs width, widths→1024, no MC) — random coordinate-spike models `M = W + e₁e₁ᵀ` (**no training**): `mc_*.npz` MC references (format shared with `checkpoints/binned_kprop/`), `mc2_*.npz` MC with split halves (cross-MSE), `pred/*.npz` cached prediction vectors (keyed by net+config, independent of the MC budget — re-scoreable), `pts/*.json` resumable scalar points, `r2/r2v2_*.json` per-(width,seed) affine-R² rows (v2: `num_nodes` = positive-side budget; old `r2_*` rows are pre-2026-07-15, not reused) |
| `checkpoints/affine_conditional_layer1/` | **affine-conditional "why" suite** (`experiments/affine_conditional_layer1/`) — random `M = W + e₁e₁ᵀ` (**no training**). Layer-1 notebook: bulk-given-spike post-ReLU stats via EXACT rectified-Gaussian integrals (no MC) → affine 1−R² ∝ 1/n, closed-form constants (`aff_l1_*.npz`, `mc_l1_*.npz`). Layer-2 knee scripts `knee_{theory,mc,analysis}.py` (`knee2_*`): PRE non-affineness = Tweedie/Bayes knee, width-flat, rank-1, cov S-curve verified @n=2048. Depth scripts `knee_depth_{mc,analysis}.py` (`knee3_*`, depth-3, n≤1024): knee = fixed-point of the 1-d spike-marginal recursion `p_{ℓ+1}=φ_ω*(c·ReLU+μ)_#p_ℓ`, damped propagation q≈0.5, no depth blow-up. Channel scripts `knee_channel_{mc,analysis}.py` (`chanmc_*`, n≤2048): recursion marginal accuracy D²∝n⁻¹ (κ=wᵀm₁ owns the error; +κ → n⁻¹·⁸), layer-2 closed form (atom+skew-normal). **Summary PDF: `writeups/affine_knee/affine_knee_summary.pdf`** |
