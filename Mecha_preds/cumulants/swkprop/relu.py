"""relu.py -- Gaussian-ReLU moments for SW-KPROP (numpy + scipy, torch-free).

The exact rank<=2 nonlinear step of the shifted-weight algorithm needs two closed
forms (paper eqs 22-24):

    rank 1 (one neuron, ``Y ~ N(mu, var)``):
        E[ReLU(Y)]   = mu*Phi(a) + sigma*phi(a)              a = mu/sigma
        E[ReLU(Y)^2] = (mu^2 + var)*Phi(a) + mu*sigma*phi(a)

    rank 2 (a pair ``(Z_i, Z_j) ~ N(mu, Sigma)``):
        E[ReLU(Z_i) ReLU(Z_j)]  via the exact bivariate-normal moments
        (Owen's T for Phi_2), then Cov = E[..] - E[ReLU_i]E[ReLU_j].

These are exactly the formulas implemented (and validated to ~1e-15 against
quadrature + 20M-sample MC) in ``kprop.exact_relu_covariance``; they are
reproduced here self-contained in numpy/scipy ONLY -- no torch import -- so the
SW-KPROP core stays importable and testable with numpy alone (the same choice
``exact_meanprop`` makes). Theorem note: at rank<=2 the ReLU step is exact, so
the certified quadrature tolerance is tau = 0.

See ``kprop/exact_relu_covariance.py`` for the full derivation and the numerics
notes (deterministic-coordinate handling, |rho|->1 limits, Owen's-T validation).
"""
